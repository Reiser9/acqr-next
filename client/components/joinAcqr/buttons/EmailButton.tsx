import EmailButtonProps from '../../../interfaces/EmailButtonProps'
import TwitterButtonProps from '../../../interfaces/twitterButtonProps'
import LoaderSmall from '../../svgs/LoaderSmall'
import InvalidEmail from '../../warnings/InvalidEmail'
...
export default function EmailButton({
  ...
  return (
    <>
      {/* EMAIL */}
      <div className="join__button mt5">
        <button
          className="join__button--content"
          onClick={() => (emailVerified ? '' : HideShowDropDown('email'))}
        >
          <div
            className={classNames(
              'join__inner--button purple__button',
              dropDowns.includes('email') ? ' rounded-t-xl' : ' rounded-xl'
            )}
          >
            <div className="join__button--social">
              ...
              >
                <path
                  d="M11.307 8.774a.911.911 0 0 1-1.102 0l-7.837-5.92L.504 1.446V15.04c0 .53.418.96.932.96h18.64c.515 0 .932-.43.932-.96V1.446l-1.864 1.408-7.837 5.92Z"
                  fill="#fff"
                />
                <path d="M10.756 6.81 19.771 0H1.741l9.015 6.81Z" fill="#fff" />
              </svg>

              <span className="join__button--text">Enter Email Address</span>
            </div>
        ...
        <div
          className={classNames(
            'join__drop',
            dropDowns.includes('email') ? '' : 'hidden'
          )}
        >
          <div>
            {failedEmailVerification ? (
